!function ($) {
  ...
  BootstrapTable.prototype.initToolbar = function () {
    ...
    if (this.options.showRefresh) {
      html.push(sprintf('<button class="btn btn-default" type="button"'+
        ' name="refresh" title="%s">',
      ...
      $.each(this.options.columns, function (i, column) {
        ...
        if (column.switchable) {
          html.push(sprintf('<li>' +
            '<label><input type="checkbox" data-field="%s" '+
            'value="%s"%s> %s</label>' +
  ...
  BootstrapTable.prototype.initPagination = function () {
    ...
    $.each(pageList, function (i, page) {
      if (that.options.smartDisplay === false || that.options.totalRows >= page || page == pageList[0]) {
        ...
        pageNumber.push(sprintf('<li%s><a href="javascript:void(0)"'+
          '>%s</a></li>', active, page));
      }
    });
    ...
    for (i = from; i <= to; i++) {
      html.push('<li class="page-number' + (i === this.options.pageNumber ?
        ' active disabled' : '') + '">',
